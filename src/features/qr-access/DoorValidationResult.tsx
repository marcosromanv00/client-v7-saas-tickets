...
export function DoorValidationResult({ outcome, ticket, errorMessage, onDismiss }: DoorValidationResultProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-[#0b1a30] rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden border border-slate-200 dark:border-[#1e355b] animate-in zoom-in-95 duration-150 text-center text-slate-900 dark:text-slate-100 transition-colors">
        {/* Resultado: Éxito */}
          ...
          <div className="p-8 space-y-5">
            ...
            <div className="bg-slate-50 dark:bg-[#071324] p-4 rounded-2xl border border-slate-200 dark:border-[#1a3357] text-left space-y-2">
              <div className="flex items-center gap-2 text-xs text-slate-800 dark:text-white">
                <Armchair className="w-4 h-4 text-[#004ea2] dark:text-blue-400" />
                <span className="font-bold text-[#004ea2] dark:text-blue-400">{ticket.seatLabel || "Aforo General"}</span>
        ...
        {outcome === "ALREADY_CHECKED_IN" && ticket && (
          <div className="p-8 space-y-5 bg-amber-50/50 dark:bg-amber-950/20">
            ...
            <div className="bg-slate-50 dark:bg-[#071324] p-3 rounded-2xl border border-slate-200 dark:border-[#1a3357] text-xs text-slate-700 dark:text-slate-300 text-left">
              ...
              <p className="font-semibold mt-0.5 text-[#004ea2] dark:text-blue-400">Butaca: {ticket.seatLabel || "General"}</p>
            </div>
