...
import { ScanLine, Camera, KeyRound, Check, Zap } from "lucide-react";
import { Ticket } from "../tickets/types";

interface DoorOpticalScannerSectionProps {
  eventTickets: Ticket[];
  onProcessScan: (code: string) => void;
}

export const DoorOpticalScannerSection: React.FC<DoorOpticalScannerSectionProps> = ({
  eventTickets,
  onProcessScan,
}) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {/* Visor de Cámara */}
      <div className="bg-white dark:bg-[#0b1a30] rounded-3xl p-6 border border-slate-200 dark:border-teatro-navy-border flex flex-col items-center justify-center relative min-h-80 shadow-xs">
        <div className="relative w-60 h-60 border-2 border-dashed border-teatro-blue/40 dark:border-blue-400/40 rounded-3xl flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-[#071324]">
          ...
          <div className="z-10 text-center px-4">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Enfoque el QR impreso/móvil o digite el código
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="bg-white dark:bg-[#0b1a30] p-6 rounded-3xl border border-slate-200 dark:border-teatro-navy-border shadow-xs">
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-3">
            Escriba el código corto (ej: <strong>AL14</strong>). Se valida automáticamente al escribir el 4to carácter.
          </p>
        ...
        {/* Acceso Rápido a Pases Recientes */}
        <div className="bg-white dark:bg-[#0b1a30] p-5 rounded-3xl border border-slate-200 dark:border-teatro-navy-border text-xs shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-slate-900 dark:text-white">Pases Emitidos para esta Función</span>
            <span className="text-[10px] text-teatro-blue dark:text-blue-400 font-mono font-semibold">
              {eventTickets.length} pases
            </span>
          </div>
          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
            {eventTickets.slice(0, 5).map((tkt) => (
              <button
                key={tkt.id}
                onClick={() => onProcessScan(tkt.shortCode || tkt.qrCodeValue)}
                className="w-full text-left p-2 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-teatro-blue dark:hover:border-blue-500 hover:bg-teatro-blue-light dark:hover:bg-teatro-blue/15 flex items-center justify-between transition-colors cursor-pointer"
              >
