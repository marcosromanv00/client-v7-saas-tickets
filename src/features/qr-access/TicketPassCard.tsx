...
export function TicketPassCard({ ticket, event }: TicketPassCardProps) {
  ...
  return (
    ...
    >
      <div className="relative bg-white dark:bg-[#0b1a30] rounded-3xl border border-slate-200 dark:border-[#1e355b] shadow-xl overflow-hidden transition-colors">
        ...
        <div className="relative h-44 sm:h-48 overflow-hidden">
          ...
          <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-950/70 backdrop-blur-md border border-white/10 text-white text-[10px]">
              <Landmark className="w-3.5 h-3.5 text-[#c59223]" />
              <span>Teatro Municipal de Alajuela</span>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono font-semibold ${
              ticket.checkedIn ? "bg-emerald-500/30 text-emerald-300 border border-emerald-500/40" : "bg-[#004ea2] text-white"
            }`}>
        ...
        <div className="relative flex items-center h-8 bg-white dark:bg-[#0b1a30]">
          <div className="absolute -left-3.5 w-7 h-7 rounded-full bg-slate-50 dark:bg-[#040b17] border-r border-slate-200 dark:border-[#1e355b]" />
          <div className="w-full border-t-2 border-dashed border-slate-300 dark:border-slate-700 mx-6" />
          <div className="absolute -right-3.5 w-7 h-7 rounded-full bg-slate-50 dark:bg-[#040b17] border-l border-slate-200 dark:border-[#1e355b]" />
        </div>
        ...
        <div className="p-6 pt-1 space-y-4 text-center">
          <div className="grid grid-cols-3 gap-2 text-left bg-slate-50 dark:bg-[#071324] p-3 rounded-2xl border border-slate-200 dark:border-[#1a3357] text-[11px]">
            <div>
              <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1"><Calendar className="w-3 h-3 text-[#004ea2] dark:text-blue-400" /> Fecha</span>
              <span className="font-semibold text-slate-800 dark:text-slate-200 block mt-0.5">{event.date}</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3 text-[#004ea2] dark:text-blue-400" /> Hora</span>
              <span className="font-semibold text-slate-800 dark:text-slate-200 block mt-0.5">{event.time}</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1"><Armchair className="w-3 h-3 text-[#004ea2] dark:text-blue-400" /> Butaca</span>
              <span className="font-mono font-bold text-[#004ea2] dark:text-blue-400 block mt-0.5">
          ...
          <div className="flex flex-col items-center justify-center p-3 bg-white rounded-2xl max-w-44 mx-auto shadow-xs border border-slate-200">
            {qrUrl ? (
      ...
      <div className="mt-4 flex items-center justify-between gap-3 print:hidden">
        <motion.button
          ...
          type="button"
          onClick={handleShare}
          className="flex-1 py-3 px-4 bg-white dark:bg-[#0b1a30] hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-2xl border border-slate-200 dark:border-[#1e355b] text-xs font-semibold flex items-center justify-center gap-2 transition-colors cursor-pointer shadow-xs"
        >
          {copied ? <Check className="w-4 h-4 text-emerald-600" /> : <Share2 className="w-4 h-4 text-[#004ea2] dark:text-blue-400" />}
        ...
        <motion.button
          ...
          type="button"
          onClick={handlePrint}
          className="flex-1 py-3 px-4 bg-[#004ea2] hover:bg-[#003c80] text-white font-semibold rounded-2xl text-xs flex items-center justify-center gap-2 shadow-md shadow-blue-900/20 transition-all cursor-pointer"
        >
