...
export const Step2SeatSelection: React.FC<Step2SeatSelectionProps> = ({
  ...
  return (
    <div className="space-y-3 sm:space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-white dark:bg-[#0b1a30] p-2.5 sm:p-3.5 rounded-2xl border border-slate-200 dark:border-[#1e355b] shadow-xs transition-colors">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
              ...
              <span>•</span>
              <span className="font-mono text-[#004ea2] dark:text-blue-400 font-semibold">{selectedTime} hrs</span>
              <span>•</span>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs px-2.5 py-1 rounded-full bg-[#004ea2]/10 text-[#004ea2] dark:text-blue-400 border border-[#004ea2]/25 font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        <div className="hidden lg:block lg:col-span-4 sticky top-20">
          <div className="bg-white dark:bg-[#0b1a30] border border-slate-200 dark:border-[#1e355b] rounded-2xl p-4 sm:p-5 shadow-sm space-y-4 transition-colors">
            <div className="flex items-center gap-3 pb-3 border-b border-slate-200 dark:border-[#1e355b]">
              <img src={event.posterUrl} alt={event.title} className="w-14 h-20 rounded-xl object-cover shrink-0 shadow-xs" />
              <div>
                <span className="text-[10px] font-mono text-[#004ea2] dark:text-blue-400 uppercase tracking-wider font-semibold">{event.genre}</span>
              ...
              {selectedSeats.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
                  {selectedSeats.map((s) => (
                    <span
                      key={s.id}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-mono bg-[#004ea2]/10 text-[#004ea2] dark:text-blue-400 border border-[#004ea2]/25 font-semibold"
                    >
                      <span>{s.label}</span>
                      <button
                        type="button"
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-[#1e355b] space-y-2">
              ...
              <div className="flex items-center justify-between text-sm font-semibold text-slate-900 dark:text-white">
                ...
                <span className="font-mono text-base text-[#004ea2] dark:text-blue-400 font-bold">{event.isPrivate ? "₡0 (Acceso Libre)" : `₡${totalPrice.toLocaleString("es-CR")}`}</span>
              </div>
            </div>
            ...
            <motion.button
              ...
              className={`w-full py-3.5 rounded-2xl text-xs font-semibold flex items-center justify-center gap-2 transition-all shadow-md ${
                selectedSeatIds.length > 0
                  ? "bg-[#c8102e] hover:bg-[#a60c25] text-white shadow-red-900/20 cursor-pointer"
      ...
      <div className="block lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/95 dark:bg-[#0b1a30]/95 backdrop-blur-xl border-t border-slate-200 dark:border-[#1e355b] z-40 shadow-lg">
        <div className="max-w-md mx-auto flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl text-xs font-semibold flex items-center gap-2 transition-all ${
              selectedSeatIds.length > 0
                ? "bg-[#c8102e] hover:bg-[#a60c25] text-white shadow-md shadow-red-900/25 cursor-pointer"
                : "bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed"
            }`}
          >
            <span>Continuar</span>
