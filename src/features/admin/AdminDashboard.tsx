...
import { Lock, LogIn, Sliders, RotateCcw, Shield, Users, History, LayoutGrid, Tag } from "lucide-react";
...
import { BraceletManagerModal } from "./BraceletManagerModal";
...
export function AdminDashboard({ onOpenLoginModal }: AdminDashboardProps) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-6">
        ...
        <button
          ...
          className="px-6 py-3 bg-muni-red hover:bg-muni-red-hover text-white font-semibold rounded-2xl text-sm inline-flex items-center gap-2 shadow-md shadow-red-900/20 transition-all cursor-pointer"
        >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-28 space-y-7 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#0b1a30] p-6 rounded-3xl border border-slate-200 dark:border-teatro-navy-border shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono uppercase text-teatro-blue dark:text-blue-400 tracking-widest font-semibold">Capa Interna • Producción & Protocolo</span>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-muni-red-light dark:bg-red-950/50 text-muni-red dark:text-red-400 font-mono font-semibold border border-muni-red/20">
              {currentUser?.name}
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex bg-slate-100 dark:bg-[#071324] p-1 rounded-2xl border border-slate-200 dark:border-[#1a3357] text-xs">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-semibold transition-colors cursor-pointer ${subTab === "aforo" ? "bg-teatro-blue text-white shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
            >
              <Shield className="w-3.5 h-3.5" /> Aforo
            </button>
            <button
              onClick={() => setSubTab("matriz")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-semibold transition-colors cursor-pointer ${subTab === "matriz" ? "bg-teatro-blue text-white shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
            >
            ...
            {isSuperAdmin && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-semibold transition-colors cursor-pointer ${subTab === "admins" ? "bg-teatro-blue text-white shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
              >
            ...
            {(isSuperAdmin || isProducer) && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-semibold transition-colors cursor-pointer ${subTab === "auditoria" ? "bg-teatro-blue text-white shadow-xs" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
              >
          ...
          {(subTab === "aforo" || subTab === "matriz") && (
            <>
              ...
              <button
                onClick={() => setIsBraceletManagerOpen(true)}
                className="px-3 py-1.5 bg-slate-100 dark:bg-[#071324] hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-[#1a3357] font-semibold rounded-xl text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
                title="Administrar catálogo de brazaletes de entrada"
              >
                <Tag className="w-3.5 h-3.5 text-teatro-blue dark:text-blue-400" /> Brazaletes
              </button>

              <button
                onClick={() => setIsConfigOpen(true)}
                className="px-3.5 py-1.5 bg-teatro-blue hover:bg-teatro-blue-hover text-white font-semibold rounded-xl text-xs flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
              >
      ...
      <BraceletManagerModal
        isOpen={isBraceletManagerOpen}
        onClose={() => setIsBraceletManagerOpen(false)}
        colors={store.braceletColors || []}
        onSave={(updated) => store.updateBraceletCatalog(updated)}
      />
    </div>
  );
}
