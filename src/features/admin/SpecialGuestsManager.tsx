...
export function SpecialGuestsManager({ event }: SpecialGuestsManagerProps) {
  ...
  return (
    <div className="bg-[#0e1626]/90 backdrop-blur-md rounded-3xl border border-slate-800 p-6 shadow-xl space-y-5 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-amber-400" />
            <h3 className="font-serif text-base text-white font-medium">Lista Especial y Cupos de Protocolo</h3>
          </div>
        ...
        <button
          ...
          className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-1.5 transition-colors self-start sm:self-auto shadow-md shadow-amber-500/20"
        >
      ...
      {isAdding && (
        <form onSubmit={handleAddGuest} className="p-5 bg-slate-900/90 rounded-2xl border border-slate-800 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
            </div>
            <div>
              <label className="block text-[11px] font-mono text-slate-300 mb-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-[11px] font-mono text-slate-300 mb-1">Cupos Reservados</label>
              <input
                type="number"
                min="1"
                max="20"
            ...
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-mono text-slate-300 mb-1">Notas de Protocolo / Ubicación</label>
              <input
                type="text"
                value={notes}
        ...
        {eventGuests.map((guest) => {
          ...
          return (
            <div key={guest.id} className="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-white">{guest.name || "Bloque sin nombre asignado"}</span>
                  <span className="px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 border border-amber-400/30 font-mono text-[10px]">
              ...
              <div>
                <button
                  ...
                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-950 border border-slate-800 text-slate-200 disabled:text-slate-600 rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5"
                >
