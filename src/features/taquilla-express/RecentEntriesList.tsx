...
export function RecentEntriesList({ entries }: RecentEntriesListProps) {
  return (
    <div className="bg-white dark:bg-[#0b1a30] rounded-3xl border border-slate-200 dark:border-[#1e355b] p-6 text-left shadow-sm transition-colors">
          ...
          {entries.map((entry) => (
            <div key={entry.id} className="py-3 flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400">
                <span className="font-mono text-[#004ea2] dark:text-blue-400 font-semibold">
                  {entry.seatLabel || (entry.zone === "PLANTA_BAJA" ? "Platea" : "Balcón")}
                </span>
                <span className="flex items-center gap-1 font-mono text-[11px] text-slate-400">
                  <Clock className="w-3 h-3 text-[#c59223]" />
